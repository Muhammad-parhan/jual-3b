...
class kategorictrl extends Controller
{
    ...
    $kategori = DB::table("kategoris")->orderBy($request->A,'asc')->paginate(4);
    
    return view("kategori.awal",["kategori" => $kategori]);
    }else if($request->has('B'))
    {
        $kategori = DB::table("kategoris")->orderBy($request->B,'desc')->paginate(4);
        return view("kategori.awal",["kategori" => $kategori]);

    }else if ($request->has('find')){
        $kategori = DB::table("kategoris")->where("nama","like","%".$request->find."%")->paginate(4);
        return view("kategori.awal",["kategori" => $kategori]);

    }else{
        $kategori = DB::table("kategoris")->paginate(4);
        return view("kategori.awal",["kategori" => $kategori]);
